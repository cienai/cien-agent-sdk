...
class AdminSyncMappingsNewAPI(EndpointGroup):
    """/api/admin/sync-mappings-new endpoints."""

    def get_mapping_type(self, sync_id: int) -> dict[str, str]:
        """Fetch the current mapping type for one sync."""
        return self._get(f"/api/admin/sync-mappings-new/{sync_id}/mapping-type")

    def set_mapping_type(self, sync_id: int, *, mapping_type: str) -> dict[str, str]:
        """Update the mapping type for one sync."""
        return self._put(
            f"/api/admin/sync-mappings-new/{sync_id}/mapping-type",
    ...
    def set_mapping(
        ...
        *,
        ...
        return self._put(
            f"/api/admin/sync-mappings-new/{sync_id}/mappings/{crm_entity}",
