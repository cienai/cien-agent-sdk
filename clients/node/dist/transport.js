...
function isTokenExpiredPayload(payload) {
    const msg = extractMessage(payload, '').toLowerCase();
    return (msg.includes('token_expired') ||
    ...
    async request(method, path, options = {}) {
        ...
        try {
            ...
            if (response.status >= 400) {
                    ...
                    try {
                        ...
                        if (refreshed) {
                            ...
                            if (retryResp.status >= 400) {
                                ...
                                throw new APIError(retryResp.status, extractMessage(retryPayload, retryResp.statusText || 'Request failed'), retryPayload);
                            }
                            // successful retry
                            ...
                            if (retryContentType.includes('application/json')) {
                                return hydrateJsonValue((await retryResp.json()));
                            }
                ...
                throw new APIError(response.status, extractMessage(payload, response.statusText || 'Request failed'), payload);
            }
            ...
            if (contentType.includes('application/json')) {
                return hydrateJsonValue((await response.json()));
            }
